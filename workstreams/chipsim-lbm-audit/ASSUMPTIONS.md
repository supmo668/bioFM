...
| **A7** | Ligands are **exchangeable**, so bootstrap-over-ligands is valid. | **Optimistic.** A ~40-compound set assembled from ChEMBL against transporters will contain **analog series**, which are not independent draws. Effective n is then below nominal n, CIs are wider than simulated, and power is lower. R5's own MMP machinery could measure this — the fraction of the set in matched series is computable locally. **Not yet done.** | OPEN |
