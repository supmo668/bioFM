...
description: Perturb-seq-eval worktree agent — substance + quality gate for the perturb-seq-eval module. Thin wrapper that @-includes the generic worktree-agent class for the module worktree; principal-agnostic (resolves at runtime from $USER).
---
...
<!--
  ...
  `perturb-seq-eval.md` (bare, no `-agent` suffix), one per app module, replacing the placeholders below.
  The plugin ships ONLY this template — no named worktree-agent instances, no
  real module names.

  Placeholders:
    perturb-seq-eval      the agent slug == the module name (e.g. "web")
    perturb-seq-eval     human-readable module label (e.g. "Web")
    perturb-seq-eval   the worktree dir under paths.worktree_base (collapse rule applies)
...
# Perturb-seq-eval Worktree Agent

A **named worktree-agent instance** for the `perturb-seq-eval` module, bound to the
...
- **Agent slug:** `perturb-seq-eval` (matches the module name).
...
- **Worktree:** `perturb-seq-eval/` under `paths.worktree_base`. Identity is bound by
  the `.aiadlc-agent` file written by `worktree-create`.
- **Workstream:** `perturb-seq-eval` under `paths.workstreams_root` (receipts, KNOWLEDGE).
- **Handoff:** under `<sandbox_root>/` for the runtime-resolved principal, at
  `perturb-seq-eval/perturb-seq-eval-handoff.md`.
