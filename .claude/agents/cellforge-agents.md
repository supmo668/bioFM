---
name: cellforge-agents
description: Cellforge-agents worktree agent — substance + quality gate for the cellforge-agents module. Thin wrapper that @-includes the generic worktree-agent class for the module worktree; principal-agnostic (resolves at runtime from $USER).
---
...
<!--
  ...
  `cellforge-agents.md` (bare, no `-agent` suffix), one per app module, replacing the placeholders below.
  The plugin ships ONLY this template — no named worktree-agent instances, no
  real module names.

  Placeholders:
    cellforge-agents      the agent slug == the module name (e.g. "web")
    cellforge-agents     human-readable module label (e.g. "Web")
    cellforge-agents   the worktree dir under paths.worktree_base (collapse rule applies)
...
# Cellforge-agents Worktree Agent

A **named worktree-agent instance** for the `cellforge-agents` module, bound to the
...
- **Agent slug:** `cellforge-agents` (matches the module name).
...
- **Worktree:** `cellforge-agents/` under `paths.worktree_base`. Identity is bound by
  the `.aiadlc-agent` file written by `worktree-create`.
- **Workstream:** `cellforge-agents` under `paths.workstreams_root` (receipts, KNOWLEDGE).
- **Handoff:** under `<sandbox_root>/` for the runtime-resolved principal, at
  `cellforge-agents/cellforge-agents-handoff.md`.
