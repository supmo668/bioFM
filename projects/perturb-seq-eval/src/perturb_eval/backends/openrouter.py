...
class OpenRouterBackend:
    ...
    def complete(
        ...
        *,
        ...
        For reasoning-model outputs (e.g. ``nvidia/nemotron-*``) OpenRouter
        splits the response across ``message.content`` and ``message.reasoning``.
        We concatenate both so downstream parsers can find e.g. a
        ``FINAL=<float>`` sentinel regardless of which field the model chose.
        """
        body = {
            "model": self.model,
            "messages": [
